const { Orden_empleado } = require("../models");
...
module.exports = {
    ...
    try {
      ...
      if (ID == 0) {
        ...
        const newRecord = await db.sequelize.query(
          `EXEC [${process.env.DB_NAME2}].[dbo].[sp_InsertarOrdenEmpleado] :NUM_ORDEN, :CODIGO_EMP, :UNIDADES_PROD, :TURNO, :CENTRO, :CODIGO, :MAQUINA`,
          {
  ...
  getAll: async (req, res) => {
    try {
      const records = await Orden_empleado.findAll({
        include: [],
        order: [["ID", "ASC"]],
      });
  ...
  getById: async (req, res) => {
    try {
      const { id } = req.params;
      const record = await Orden_empleado.findByPk(id, {
        include: [],
      });
